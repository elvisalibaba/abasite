  ...
  return (
    <>
      ...
      <div className="admin-content admin-module-page" data-module={view}>
        ...
        <AdminSection id="users" title="Comptes utilisateurs" subtitle={isAdmin ? "Gérez les accès et les rôles" : "Section réservée aux administrateurs"} actionLabel={isAdmin ? "Créer un compte" : undefined} onAction={() => setModal({ type: "user" })}>
      ...
      {modal && <div className="admin-modal-backdrop" onMouseDown={() => setModal(null)}><div className="admin-modal" onMouseDown={event => event.stopPropagation()}><button className="admin-modal-close" onClick={() => setModal(null)}>×</button>
        ...
        {modal.type === "user" && <EntityForm title="Créer un compte" subtitle="L’utilisateur pourra se connecter immédiatement." onSubmit={form => submit(form, createUserAction)} pending={pending}><Field label="Nom complet *"><input name="full_name" required /></Field><div className="admin-form-row"><Field label="Adresse e-mail *"><input name="email" type="email" required /></Field><Field label="WhatsApp *"><input name="whatsapp_phone" type="tel" placeholder="+243…" required /></Field></div><div className="admin-form-row"><Field label="Mot de passe temporaire *"><input name="password" type="password" minLength={8} required /></Field><Field label="Rôle"><select name="role"><option value="personnel">Personnel</option><option value="chef_projet">Chef de projet</option><option value="garde">Garde</option><option value="direction">Direction</option><option value="admin">Administrateur</option></select></Field></div></EntityForm>}
      </div></div>}
    </>
  );
}
