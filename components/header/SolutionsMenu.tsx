...
export function SolutionsMenu({ mobile = false, onNavigate }: { mobile?: boolean; onNavigate?: () => void }) {
  if (mobile) return <div className="mobile-solutions-list">{solutionGroups.map(group=><section key={group.label}><h2>{group.label}</h2>{group.items.map(([title,href])=><Link href={href} onClick={onNavigate} key={href}>{title}<span aria-hidden="true">→</span></Link>)}</section>)}</div>;
